...
const loginUser = async (req, res) => {
    ...
    try {
        ...
        if (process.env.NODE_ENV === 'test') {
            return res.status(200).json({ message: 'OTP sent to your email', userId: user._id, token: otp });
        }
...
const forgotPassword = async (req, res) => {
    ...
    try {
        ...
        await user.save();
        if (process.env.NODE_ENV === 'test') {
            return res.status(200).json({ message: 'Email sent', token: resetPasswordToken });
        }
...
module.exports = { registerUser, loginUser, verifyOtp, verifyEmail, forgotPassword, resetPassword, logout };